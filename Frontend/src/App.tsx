...
// ВКАЖИ ТУТ ПОРТ СВОГО C# СЕРВЕРА
...
export default function App() {
  ...
  const [token, setToken] = useState<string | null>(null); // Стан для збереження JWT токена
  ...
  // === 2. ВХІД ТА ОТРИМАННЯ ТОКЕНА ===
  const handleUnlock = async () => {
    try {
      const currentUserId = 1; // MVP: використовуємо ID 1
      
      const res = await axios.post(`${API_URL}/unlock`, { userId: currentUserId, masterPassword });
      
      // Зберігаємо розшифровані дані ТА токен доступу
  ...
  // === 3. ДОДАВАННЯ ПАРОЛЯ (Захищено JWT) ===
  const handleAddPassword = async () => {
    try {
      // Прикріплюємо JWT токен до заголовків запиту
      const config = {
      ...
      }, config); // Передаємо конфіг з токеном
      ...
      alert("Помилка додавання пароля (Можливо, термін дії токена минув)");
    }
  };
  ...
  const handleSecureCopy = (text: string) => {
    ...
    setTimeout(() => {
      ...
      console.log("Буфер очищено з міркувань безпеки");
    }, 30000);
  };

  // === РЕНДЕР ЕКРАНА АВТОРИЗАЦІЇ ===
  if (!userId) {
    return (
      <Container maxWidth="sm" sx={{ mt: 10 }}>
        <Card elevation={4}>
          <CardContent sx={{ p: 4, display: 'flex', flexDirection: 'column', gap: 2 }}>
            ...
            <TextField 
              label="Логін" variant="outlined" fullWidth 
    ...
    <Box sx={{ flexGrow: 1, bgcolor: '#f5f5f5', minHeight: '100vh' }}>
      <AppBar position="static">
        <Toolbar>
          ...
          <Button color="inherit" onClick={() => { setUserId(null); setVault([]); setSessionPassword(''); setToken(null); }}>
            Вийти
          </Button>
        </Toolbar>
      </AppBar>
      ...
      <Container maxWidth="md" sx={{ mt: 4 }}>
        ...
        <Paper elevation={2} sx={{ p: 3, mb: 4, display: 'flex', gap: 2, alignItems: 'center' }}>
          ...
          <Button variant="outlined" onClick={handleGeneratePassword} sx={{ minWidth: '40px', px: 1 }} title="Згенерувати пароль">
            🎲
          </Button>
            ...
            <Card key={record.id} elevation={1}>
              <CardContent sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', p: 2, '&:last-child': { pb: 2 } }}>
                ...
                <Box sx={{ display: 'flex', gap: 1, alignItems: 'center' }}>
                  ...
                  <Button size="small" variant="contained" color="secondary" onClick={() => handleSecureCopy(record.password)}>
                    Копіювати
                  </Button>
                </Box>
              </CardContent>
